#### 4-7-23 yearly-aggregated analysis #####
# should be a better replication of Wang et al 2016

source("code/dev_yearlyaggregation_quickprep.R")

main_mod <-
    feglm(
        n_deaths ~
            mean_pm2.5 +
            ns(temp_summer, df=3) + ns(temp_winter, df=3) +
            precip_summer + precip_winter | fipsihme + year,
        offset = log(final$pop),
        data = final,
        weights = final$pop,
        family = quasipoisson,
        cluster = final$fipsihme)

summary(main_mod)
interpret_model(main_mod, "cluster")

noprecip <-
    feglm(
        n_deaths ~
            mean_pm2.5 +
            ns(temp_summer, df=3) + ns(temp_winter, df=3) | fipsihme + year,
        offset = log(final$pop),
        data = final,
        weights = final$pop,
        family = quasipoisson,
        cluster = final$fipsihme)

lineartemp <-
    feglm(
        n_deaths ~
            mean_pm2.5 +
            temp_summer + temp_winter +
            precip_summer + precip_winter | fipsihme + year,
        offset = log(final$pop),
        data = final,
        weights = final$pop,
        family = quasipoisson,
        cluster = final$fipsihme)

no_weighting <-
    feglm(
        n_deaths ~
            mean_pm2.5 +
            ns(temp_summer, df=3) + ns(temp_winter, df=3) +
            precip_summer + precip_winter | fipsihme + year,
        offset = log(final$pop),
        data = final,
        family = quasipoisson,
        cluster = final$fipsihme)


# Forest plot (base code from Jeff's SEDA paper)
mod_list <-
    rbind(
        main_mod$coeftable %>% data.frame() %>% mutate(mod_name = "main", var=rownames(.)),
        lineartemp$coeftable %>% data.frame() %>% mutate(mod_name = "lineartemp", var=rownames(.)),
        noprecip$coeftable %>% data.frame() %>% mutate(mod_name = "noprecip", var=rownames(.)),
        no_weighting$coeftable %>% data.frame() %>% mutate(mod_name = "no_weighting", var=rownames(.))
    ) %>% 
    dplyr::select(-`t.value`, -`Pr...t..`)

mod_df <- 
    mod_list %>%
    filter(grepl("mean_pm2.5", var)) %>% 
    rename(est=Estimate, se=`Std..Error`) %>% 
    mutate(
        point_est = exp(est),
        CI_lower = exp(est - qnorm(0.975) * se),
        CI_upper = exp(est + qnorm(0.975) * se),
        
        mod_name = factor(mod_name, levels = c("main", "lineartemp", "noprecip", "no_weighting"))
    )

yearlyagg_regs <-
    ggplot(data = mod_df) + #exponentiate to get the risk increase for 1 µg/m3 wfpm2.5
    base_theme + 
    theme(
        legend.position = "none"
    ) + 
    geom_hline(yintercept = 0, colour = DEFAULT_COLOR, lty = 2, size=0.25) + 
    geom_linerange(aes(x=mod_name, ymin=CI_lower-1, ymax=CI_upper-1), lwd=0.75,position=position_dodge2(width=.25), color=LINE_COLOR) + 
    geom_point(aes(x=mod_name, y=point_est-1), position=position_dodge2(width=.25), shape = 21, fill = "WHITE", color=LINE_COLOR, size=3) + 
    labs(
        title="Wildfire smoke PM2.5 x all-cause mortality",
        subtitle="Annual aggregation, FEs for county and year",
        y=expression(atop("%"~Delta~" Mortality Rate per", "1"~mu*"g m"^"-3")), 
        x="", 
        color="Subject"
    ) + 
    scale_y_continuous(labels=scales::percent_format(accuracy=.01, suffix="")) +
    scale_x_discrete(guide = guide_axis(n.dodge=2))



