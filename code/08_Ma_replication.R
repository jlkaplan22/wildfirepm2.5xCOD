...
ma_final_df$popw_mean_pm2.5 %>% mean() #.3887, basically same as their reported .39
ma_final_df$popw_mean_pm2.5 %>% max() #70.94726, same as theirs exactly
ma_final_df$n_deaths %>% sum() #27,811,048 which is very close to their 27,812,837

acm_model <- feglm(n_deaths ~ 
                  popw_mean_pm2.5 + 
                  ns(areaw_monthly_mean_tempF, df=5) | yearmonth + fipsihme, 
...
value_replications <-
    cbind(
        ...
        c(
            ma_final_df$popw_mean_pm2.5 %>% mean() %>% round(digits = 3) %>% format(big.mark = ","),
            ma_final_df$popw_mean_pm2.5 %>% max() %>% round(digits = 3) %>% format(big.mark = ","),
        ...
        c(
            final$popw_mean_pm2.5 %>% mean() %>% round(digits = 3) %>% format(big.mark = ","),
            final$popw_mean_pm2.5 %>% max() %>% round(digits = 3) %>% format(big.mark = ","),
